...
// Delete old toggles
for (var i=0; i<maxtog; i++) {
    var oldToggle = this.patcher.getnamed("toggleY" + i);
...
function beat(b) {
    ...
    for (var i=0; i<numToggles; i++) {
        var offToggle = this.patcher.getnamed("toggleY" + i);
            offToggle.message("bgcolor", offColor);
            offToggle.message("presentation", 1)
    

    var onToggle = this.patcher.getnamed("toggleY" + modb);
